...
private:
  ...
  void handleFileTransfer(int client_socket) {
    try {
      ...
      if (!PEM_write_bio_PUBKEY(bio, server_key.get())) {
        BIO_free(bio);
      ...
      std::unique_ptr<EVP_PKEY_CTX, decltype(&EVP_PKEY_CTX_free)> ctx(
          EVP_PKEY_CTX_new(server_key.get(), nullptr), EVP_PKEY_CTX_free);
...
public:
  SecureFileServer() : server_fd(-1), server_key(nullptr, EVP_PKEY_free) {
    if (!initializeRSA()) {
  ...
  void start() {
    while (true) {
      ...
      std::cout << "Client connected, starting file transfer..." << std::endl;
